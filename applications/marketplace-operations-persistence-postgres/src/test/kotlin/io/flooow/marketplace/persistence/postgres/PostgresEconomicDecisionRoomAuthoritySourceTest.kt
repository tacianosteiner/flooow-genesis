...
class PostgresEconomicDecisionRoomAuthoritySourceTest {
    ...
    @Test
    fun `complete persisted D4 authority chain is available to Decision Room authority reader`() {
        ...
        val decisionRoomAuthority = assertIs<EconomicDecisionRoomAuthorityRead.Available>(
            PostgresEconomicDecisionRoomAuthoritySource(dataSource).read(organizationId, created)
        ).context
        assertEquals(created.organizationId, decisionRoomAuthority.organizationId)
        assertEquals(created.caseId, decisionRoomAuthority.caseId)
        assertEquals(created.orderId, decisionRoomAuthority.marketplaceOrderId)
        assertEquals(created.traceId, decisionRoomAuthority.financialTraceId)
        assertEquals(created.policyVersion, decisionRoomAuthority.policyVersion)
        assertEquals(created.currency, decisionRoomAuthority.currency)
        assertEquals(created.revision, decisionRoomAuthority.reconciliationRevision)
        val identity = requireNotNull(decisionRoomAuthority.inputs.identity)
        val currencyAuthority = requireNotNull(decisionRoomAuthority.inputs.currency)
        val allocation = requireNotNull(decisionRoomAuthority.inputs.allocation)
        val currentness = requireNotNull(decisionRoomAuthority.inputs.currentness)
        assertEquals(EconomicTruthAuthorityState.UNRESOLVED, identity.state)
        assertEquals(EconomicTruthAuthorityState.UNRESOLVED, currencyAuthority.state)
        assertEquals(EconomicTruthAuthorityState.UNRESOLVED, allocation.state)
        assertEquals(EconomicTruthAuthorityState.UNRESOLVED, currentness.state)
        assertTrue(identity.evidenceReferences.isEmpty())
        assertTrue(currencyAuthority.evidenceReferences.isEmpty())
        assertTrue(allocation.evidenceReferences.isEmpty())
        assertTrue(currentness.evidenceReferences.isEmpty())
    ...
    @Test
    fun `missing scoped ledger entry makes authority lineage unavailable`() {
        `complete persisted D4 authority chain is available to Decision Room authority reader`()
        ...
        val durableCase = requireNotNull(
            PostgresDurableReconciliationCaseRepository(configuration).find(organizationId, caseId)
        )
        val missingEntryCase = DurableReconciliationCase(
            caseId = durableCase.caseId,
            organizationId = durableCase.organizationId,
            orderId = durableCase.orderId,
            traceId = durableCase.traceId,
            policyVersion = durableCase.policyVersion,
            currency = durableCase.currency,
            status = durableCase.status,
            openedAt = durableCase.openedAt,
            lastObservedAt = durableCase.lastObservedAt,
            resolvedAt = durableCase.resolvedAt,
            revision = durableCase.revision,
            absoluteDifferenceSummary = durableCase.absoluteDifferenceSummary,
            stages = durableCase.stages,
            evidenceEntryIds = listOf(FinancialLedgerEntryId.of(UUID(0, 94_999)))
        )

        assertIs<EconomicDecisionRoomAuthorityRead.Unavailable>(
            PostgresEconomicDecisionRoomAuthoritySource(dataSource).read(organizationId, missingEntryCase)
        )
    }

    @Test
    fun `persisted currency contradiction in authority chain is an integrity failure`() {
        `complete persisted D4 authority chain is available to Decision Room authority reader`()
        ...
        assertEquals(MarketplaceCurrency("USD"), contradictoryCase.currency)
        assertIs<EconomicDecisionRoomAuthorityRead.IntegrityFailure>(
            PostgresEconomicDecisionRoomAuthoritySource(dataSource).read(organizationId, contradictoryCase)
        )
    }

    @Test
    fun `authority chain is unavailable to a foreign organization`() {
        `complete persisted D4 authority chain is available to Decision Room authority reader`()
        ...
        val source = PostgresEconomicDecisionRoomAuthoritySource(dataSource)

        assertIs<EconomicDecisionRoomAuthorityRead.Available>(source.read(organizationA, ownCase))
        assertIs<EconomicDecisionRoomAuthorityRead.Unavailable>(source.read(organizationB, foreignCase))
    }
